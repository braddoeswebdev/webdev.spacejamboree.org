class InternetMapsController < ApplicationController
  before_action :require_login
  before_action :set_internet_map, only: [ :show ]
  before_action :require_workshop_access, only: [ :show ]
  before_action :require_instructor_or_admin, only: [ :new, :create ]

  def show
    @traceroute = @internet_map.traceroutes.new
  ...
  def new
    ...
    @internet_map = @workshop.internet_maps.new
  end

  def create
    @workshop = Workshop.find(params.expect(:workshop_id))
    @internet_map = @workshop.internet_maps.create!

    redirect_to @internet_map, notice: "Internet Map created!"
  end

  private

  def set_internet_map
    @internet_map = InternetMap.find(params.expect(:id))
  end
  ...
  def require_instructor_or_admin
    ...
    unless Current.user.admin? || @workshop.instructor == Current.user
      redirect_to @workshop, alert: "Only the instructor can manage maps."
    end
  rescue ActiveRecord::RecordNotFound
    redirect_to workshops_path, alert: "Workshop not found."
  end
